...
relationshipsApi.post('/invite', async (c: Context) => {
  try {
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'pending', datetime('now'), datetime('now', '+7 days'))
    `).bind(
      invitationId,
      inviterUserId,
      partnerEmail,
...
relationshipsApi.get('/:relationshipId/status', async (c: Context) => {
  try {
    ...
    const activityCount = await db.prepare(`
      SELECT COUNT(*) as count FROM activities WHERE relationship_id = ?
    `).bind(relationshipId).first()
...
relationshipsApi.get('/user/:userId', async (c: Context) => {
  try {
    ...
    const relationship = await db.prepare(`
      ...
      LEFT JOIN users u1 ON r.user_1_id = u1.id
      LEFT JOIN users u2 ON r.user_2_id = u2.id
      WHERE (r.user_1_id = ? OR r.user_2_id = ?) AND r.status = 'active'
...
relationshipsApi.put('/:relationshipId', async (c: Context) => {
  try {
    ...
    await db.prepare(`
      ...
      WHERE id = ?
    `).bind(relationshipType, startDate, anniversaryDate, relationshipId).run()
...
relationshipsApi.delete('/:relationshipId', async (c: Context) => {
  try {
    ...
    await db.prepare(`
      UPDATE relationships SET status = 'ended', updated_at = datetime('now') WHERE id = ?
...
relationshipsApi.post('/accept-invite', async (c: Context) => {
  try {
    ...
    await db.prepare(`
      UPDATE partner_invitations SET status = 'accepted', accepted_at = datetime('now') WHERE id = ?
